...
matrix0 = np.loadtxt(open("C:/Users/chenl/PycharmProjects/research/traning/nmf_rating_matrix.csv","rb"),delimiter=",",skiprows=0)
...
autoencoder = AdditiveGaussianAutoencoder(n_input=3883, n_hidden=100, transfer_function=tf.nn.sigmoid, optimizer=tf.train.AdamOptimizer(learning_rate=0.001), scale=0.01)

for epoch in range(training_epoch):
    avg_cost = 0
    step = 1
    cost_matrix[epoch-1][0] = epoch
    while step * batch_size < n_samples:
        ...
        step += 1
    cost_matrix[epoch-1][1] = avg_cost

print('Total cost: %0.3f' % autoencoder.calc_total_cost(X_test))
np.savetxt('nmf_auto.csv', cost_matrix, delimiter=',')